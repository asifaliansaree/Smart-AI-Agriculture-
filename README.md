...
| Task | Dataset | Algorithm(s) |
|---|---|---|
| Wheat Disease Classification | Custom wheat disease dataset (43 samples, SMOTE applied) | Decision Tree, KNN, Random Forest |
| Raisin Classification | Raisin Dataset (900 samples, 7 features) | Logistic Regression, SVM, Random Forest |
| Crop Yield Regression | FAOSTAT data (Wheat, Rice, Maize) | Linear Regression, Ridge, Random Forest, XGBoost |
| Yield Zone Clustering | Yield time-series data | K-Means (with Elbow & Silhouette analysis) |
...
```bash
python data_preprocessing.py    # Clean and preprocess all datasets
...
| Model Task | Best Model | Metric |
|---|---|---|
| Wheat Disease Classification | Random Forest | Best F1-Score (cross-validated) |
| Raisin Classification | SVM / Random Forest | ~95%+ Accuracy |
| Crop Yield Regression | XGBoost / Random Forest | Best R² per crop |
| Clustering | K-Means (optimal k) | Silhouette Score evaluated |
